...
void modeSSTF(void) {
    ...
    if (state == STOP) {
        ...
        if (car.position ==
            getPositionIndex(s_dest_stationNumber)) { // 说明到站了
            state = STOP;
        ...
        if (car.position ==
            ...
            s_dest_stationNumber = -1; // 重置
        } else if (haveRequest(COUNTERCLOCKWISE) ==
                   TRUE) { // 没到目标站但是途径站
            state = STOP;
...
void modeFCFS(void) {
    ...
    int dest_positionIndex =
        getPositionIndex(env.presentWorkingPtr->stationNumber);
    if (state == NO_TASK) {
    ...
    {
        if (car.position ==
            dest_positionIndex) //当前请求已完成，判定后续节点请求可否同时完成
        {
...
void modeSCAN(void) {
    ...
    if (state == STOP) { // stop表示停运停车状态
        ...
        */
        ...
        } else if (orient(getPositionIndex(SSTFfindNearestStationNumber())) ==
                   1) {
            //有非本站新请求，顺时针方向距离更近
            state = CLOCKWISE;
            carClockwise();
        ...
        */
        ...
        int nearestDistance =
            SCAN_stationDistance(getPositionIndex(nearestStation), 1);
        ...
        int nearestDistance =
            SCAN_stationDistance(getPositionIndex(nearestStation), 2);
        if (AreThereAnyRequest() == -1) {
            finishRequest(getStationNumber(car.position), 0, FALSE);
            state = STOP;
        } else if (nearestDistance * 2 > env.TOTAL_STATION * env.DISTANCE) {
...
int SCANfindNearestStationNumber(int state) {
    ...
    while (car.target[0][i] != -1) {
        if (car.target[0][i] == 1) {
            if (minDistance >
                SCAN_stationDistance(getPositionIndex(i + 1), state)) {
                minDistance =
                    SCAN_stationDistance(getPositionIndex(i + 1), state);
                res = i + 1;
            }
...
int stationDistance(int stationNumber) {
    ...
    counterclockwiseDistence =
        env.TOTAL_STATION * env.DISTANCE - clockwiseDistence;
...
void finishRequest(int stationNumber, int direction, int ifDelay) {
    ...
    if (car.target[0][stationNumber - 1] == 1 &&
        (car.target[1][stationNumber - 1] == 0 || !ifDelay)) {
        car.target[0][stationNumber - 1] = 0;
    }
    if (direction == 0 || direction == 1) {
        if (station.clockwise[0][stationNumber - 1] == 1 &&
            (station.clockwise[1][stationNumber - 1] == 0 || !ifDelay)) {
    ...
    if (direction == 0 || direction == 2) {
        if (station.counterclockwise[0][stationNumber - 1] == 1 &&
            (station.counterclockwise[1][stationNumber - 1] == 0 || !ifDelay)) {
    ...
    if (temp == -1) {
        ...
        } else // 逆时针
        {
            if (station.counterclockwise[0][temp - 1] == 1) {
                if (station.counterclockwise[1][temp - 1] ==
                    1) { // 如果该请求是这一秒新增的，则忽略
                    return FALSE;
                } else { // 不是这一秒新增的请求，不忽略
                    return TRUE;
...
void FCFS_haveOnStationRequest(NODE *presentWorkingPtr) {
    ...
    {
        if (getPositionIndex(presentWorkingPtr->next->stationNumber) ==
            car.position) {
